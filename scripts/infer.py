...
def parse_inference_cadences_file(filepath: str) -> list:
    """
    Parse a cadences .txt file produced by rst-build-backgrounds.

    Supports two formats:
      - mixed:    target_name|freq_start|file1,file2,...,file6
      - per-band: target_name|file1,file2,...,file6

    Returns list of dicts with keys: target_name, files (List[str]).
    """
    cadences = []
    with open(filepath) as fh:
        for lineno, line in enumerate(fh, 1):
            ...
            parts = line.split('|')
            if len(parts) == 3:
                target_name, _, files_str = parts
            elif len(parts) == 2:
                target_name, files_str = parts
            else:
                print(f"  ⚠️  Skipping malformed line {lineno}: {line!r}")
                continue
            files = files_str.split(',')
            if len(files) != 6:
                print(f"  ⚠️  Skipping line {lineno}: expected 6 files, got {len(files)}")
                continue
            cadences.append({'target_name': target_name, 'files': files})
    return cadences
...
Examples:
  ...
  python scripts/infer.py -m checkpoints/best.pth -s /data/observations/
    ...
    input_group.add_argument('--inference-cadences', '-i', type=str,
                             help='Inference cadences .txt file generated by rst-build-backgrounds')
    ...
    # ------------------------------------------------------------------ #
    #  Mode 3: Inference cadences file
    # ------------------------------------------------------------------ #
    elif args.inference_cadences:
        print(f"\n📁 Mode: Inference Cadences File ({args.inference_cadences})")
        cadence_entries = parse_inference_cadences_file(args.inference_cadences)

        if not cadence_entries:
            print("⚠️  No valid cadences found in file!")
            return
        ...
        for idx, entry in enumerate(cadence_entries, 1):
            ...
            files = entry['files']
            print(f"\n{'─' * 60}")
            print(f"  Cadence {idx}/{len(cadence_entries)}: {target}")
            print(f"{'─' * 60}")
